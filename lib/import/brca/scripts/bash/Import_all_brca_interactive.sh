...
# PROV='RQ3'
# IFS=$'\n'
# for x in $(find  $DIRPATH/$FILEPATH  -type f  \( -name "a5a6c2470626d7c3b8e8e62380e30a02288a37f8_09.2020_BRCA Challenge AZOVCA 2015_2017.xlsx.pseudo" \
#                                                  -o -name "9159d17e34ae13c12e8515f5ac930b49a3eb11a9_11.2020_BRCA Challenge BRCA to upload.xlsx.pseudo" \
#                                                  -o -name "e0e2ad35b98b2ce0d5b2a111e2fce52daef3b764_09.2022_Dec 21  _ Jul 22 BRCA.xlsx.pseudo" \
#                                                  -o -name "f7b0354fe662403eddf7acf1cb9c3cb0567dadd8_08.2022_April 20 _ Nov 21 BRCA.xlsx.pseudo" \) -path "*/$PROV/*" )
# do
# IFS="$OIFS"
# $BRAKE import:brca fname="$(echo "$x" | sed -e 's:.*pseudonymised_data/\(.*\):\1:')" prov_code=$PROV
# done
...
PROV='R0A'
IFS=$'\n'
for x in $(find  $DIRPATH/$FILEPATH -type f -name "*.pseudo" -path "*/$PROV/*" ! -name "HNPCC")
do
IFS="$OIFS"
$BRAKE import:brca fname="$(echo "$x" | sed -e 's:.*pseudonymised_data/\(.*\):\1:')" prov_code=$PROV
done
