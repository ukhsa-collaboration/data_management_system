      ...
      module Leeds
        module ReportExtractor
          ...
          class GenotypePattern < Import::ExtractionUtilities::CommonUtility
            ...
            def valid?(genotype)
              self.class::VALID_REGEX.match(genotype)
            end

            def extract(_genotype_string, _report_string, genotype)
          ...
          class AJNegative < GenotypePattern
            ...
            VALID_REGEX = /(?:predictive )?aj(?: pre-screen(?:#{"/"}conf)?)? neg(?: 3seq)?/i.freeze
          ...
          class AJPositive < GenotypePattern
            # # include ::CommonUtility
            VALID_REGEX = /(?:predictive )?aj(?: B(?:1|2))?(?: pre-screen(?:#{"/"}conf)?)? pos(?: 3seq)?/i.freeze
          ...
          class Predictive < GenotypePattern
            # include ::CommonUtility
            VALID_REGEX = /predictive b(?:rca)?(?<brca>1|2) (?<method>seq|ngs|mlpa) (?<status>pos|neg)/i.freeze

            REPORT_REGEX_NEGATIVE = /.*(?:familial)?(?<variantclass>(?: likely) pathogenic)? BRCA(?<brca>1|2) (?:mutation|sequence variant) ? (?<location>c\.[^ \.]+) (?<protein>\(p\..*\))? ?is absent.*/i.freeze
            #  @reportRegexPositive = /.*patient is (?:hetero|homo)zygous for the pathogenic BRCA(1|2) mutation (c\.[^ \.]+).*/i
            REPORT_REGEX_POSITIVE = /.*patient is (?<zygosity>hetero|homo)zygous for the ?(?<family> familial)?(?<variantclass>(?: likely)?(?: pathogenic)?) BRCA(?<brca>1|2) (?:mutation|sequence variant) ? (?<location>c\.[^ \.]+).*/i.freeze
            REPORT_REGEX_INHERITED = /has (?<status>not )?inherited the (?<family>familial )?BRCA(?<brca>1|2) (?:mutation|sequence variant) ? (?<location>c\.[^ \.]+) (?<protein>\(p\..*\))?/i.freeze
            MLPA_NEGATIVE = /mlpa.*the(?<family> familial)?(?<variantclass> (?:likely )?pathogenic)? BRCA(?<brca>1|2) (?<mutationtype>deletion|duplication) of exons? (?<exons>\d+[a-z0-9]*(?:-\d+[a-z0-9]*)?) is absent/i.freeze
            INHERITED_EXON = /has (?<status>not )?inherited the (?<family>familial )?(?<variantclass> (?:likely )?pathogenic )?BRCA(?<brca>1|2) (?<mutationtype>deletion|duplication) of exons? (?<exons>\d+[a-z0-9]*(?:-\d+[a-z0-9]*)?)/i.freeze
          ...
          class Confirmation < GenotypePattern
            # include ::CommonUtility
            VALID_REGEX = /confirmation b(?:rca)?(?<brca>1|2) (?<method>seq|ngs|mlpa) (?<status>pos|neg)/i.freeze
            def initialize
              # variant = '(?:familial)?(?<variantclass>(?: likely) pathogenic)?' \
              #           ' BRCA(?<brca>1|2) (?:mutation|sequence variant)'
              @single = /patient is #{ZYGOSITY} for the #{VARIANT} #{GENE_LOC}/i
              super
            end
            ...
            def extract(genotype_string, report_string, genotype)
              ...
              if genotype.positive?
                 ...
                 @logger.info "Something is wrong maybe" if genotype_string.include? 'neg'
                if extract_single_mutation(report_string, genotype)
          ...
          class VariantClass < GenotypePattern
            # include ::CommonUtility
            VALID_REGEX = /b(?:rca)?(?<brca>1|2) class (?<class>[1-5]a?) (uv|new)(?: unaffected patient)?/i.freeze
          ...
          class VariantSeq < GenotypePattern
            # include ::CommonUtility
            VALID_REGEX = /(?<method>ngs )?b(?:rca)?(?<brca>1|2) seq variant(?: - class (?<variantclass>[1-5]))?/i.freeze
          ...
          class ScreeningFailed < GenotypePattern
            VALID_REGEX = /^(?:(?<method>ngs) )?screening failed$/i.freeze
          ...
          class WordReportNormal < GenotypePattern
            VALID_REGEX = /^word report - normal$/i.freeze
          ...
          class DoubleNormalMLPAFail < GenotypePattern
            # include ::CommonUtility
            VALID_REGEX = /^normal #{BRCA} and #{BRCA}, MLPA fail$/i.freeze
            SEQUENCE    = /screened for #{BRCA} and #{BRCA} mutations by sequence analysis/i.freeze
            MLPA        = /mlpa analysis of #{BRCA} failed/i.freeze
          ...
          class TruncationOrFrameshift < GenotypePattern
            # include ::CommonUtility
            VALID_REGEX = /(ngs )?b(?:rca)?(1|2) truncating.frameshift/i.freeze
            IMPACT_REGEX = /(?<impact>frameshift|nonsense|missense)/i.freeze
            def extract(genotype_string, report_string, genotype)
              @report_parse_attempt_counter += 1
              genotype_matches = self.class::VALID_REGEX.match(genotype_string)
          ...
          class GenotypeAndReportExtractor
            ...
            def summary
              ...
              @logger.info format('Complete extraction successful on %0.2f percent of records', (@total_records.to_f - all_failed - @no_match) / @total_records * 100)
